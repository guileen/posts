...
exports.loadTimeline = function(userId, startIndex, len, callback) {
  ...
  msgqueue.loadTimeline(userKey, startIndex, startIndex + len, function(err, postKeys) {
      ...
      exports.loadFullPostsByKeys(userKey, postKeys, callback);
  });
}

exports.loadFullPostsByKeys = function(userKey, postKeys, callback) {
...
exports.loadFeeds = function(fids, callback) {
  Feeds.find({key : {$in : fids}}, {id: 1, favicon: 1, title: 1, key: 1}).toArray(function(err, users) {
      if(err) return callback(err);
      callback(null, users);
  });
}
...
exports.followFeeds = function(user, feeds, callback) {
  ...
  feeds.forEach(function(feed) {
      social.follow('user:' + user._id, feed.key);
  });
  ...
  tasks.push(function(_callback) {
      Users.update({ _id: db.toObjectID(user._id) }, {
          $addToSet: {
            feeds: {
              $each: feeds.map(function(feed) {return feed.key})
            }
          }
      }, _callback);
  });
...
exports.updateFeedPost = function(feedkey, article, callback) {
  ...
  Posts.findAndModify({ // query
      ...
      if(err) return callback(err);
      callback(err, doc);
      social.postItem(feedkey, post.key, {time: post.createTime.getTime()}, function(err, data) {
