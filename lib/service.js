...
exports.loadTimeline = function(userId, startIndex, len, callback) {
  msgqueue.loadTimeline('user:' + userId, startIndex, startIndex + len, function(err, postKeys) {
      if(err) return callback(err);
      exports.loadPostsByKeys(postKeys, callback);
  });
}

exports.loadPostsByKeys = function(keys, callback) {
  var guids = keys.map(function(key){ return key.substring(5 /* posts: */) });
  var query = {
    guid: {$in: guids}
  , removeTime: null
  };
...
exports.loadFeeds = function(fids, callback) {
  Feeds.find({id : {$in : fids}}, {id: 1, favicon: 1, title: 1}).toArray(function(err, users) {
      if(err) return callback(err);
      callback(null, users);
  });
}
...
exports.followFeeds = function(user, feeds, callback) {
  ...
  tasks = tasks.concat(feeds.map(function(feed) {
        return function(_callback) {
          feedqueue.addFeedToQueue(user, feed.id, _callback);
        }
  }));

  // user follow feeds
  feeds.forEach(function(feed) {
      community.follow('user:' + user._id, 'feed:' + feed.id);
  });
...
exports.updateFeedPost = function(feed, article, callback) {
  Posts.findAndModify({ // query
      ...
      if(err) return callback(err);
      callback(err, doc);
      community.postItem('feed:' + feed, 'post:' + article.guid, {}, function(err, data) {
