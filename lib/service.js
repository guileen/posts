...
exports.loadPosts = function(userId, following, sinceId, limit, callback) {
  ...
  db.posts.find(query, { revisions: { $slice : -1 }, comments: {$slice : -1}, content: -1}).sort({ _id: -1}).limit(Math.max(limit, 50)).toArray(callback);
...
exports.addComment = function(id, user, content, callback) {
  mkdToHtml(content, function(html) {
      ...
      var sign = md5([userId, content, new Date / 1000 / 30].join('&'));
      // TODO broadcast to
...
exports.loadComments = function(id, limit, callback) {
