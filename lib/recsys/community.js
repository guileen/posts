...
function print(err) {if (err) logger.error(err);}
...
// TODO refactor this
var SCRIPT_USER_PUSH_ITEM = [
  // start is the index of current operation followers pagine index
  "local user, item, start, time = KEYS[1], KEYS[2], KEYS[3], KEYS[4];"
, "redis.call('zcard', user .. ':followers');"
  // TODO use workers
, "local followers = redis.call('zrange', user .. ':followers', start, -1);"// stop should be start + 100, and pagine this operation in workers"
, "local result = redis.call('hmget', item, 'avgrate', 'type')"
, "local itemavgrate, type = result[0] or 0, result[1]"
...
exports.itemCommunityInfo = function(user, item, callback) {
  ...
  .exec(function(err, results) {
      ...
      info.isLike = results[2];
      info.isDislike = results[3];
      info.isShared = results[4];
      callback(null, info);
  });
};
...
exports.postItem = function(user, item, info, callback) {
  ...
  info.owner = user;
  var time = info.time = now();
  redis.multi()
  .hmset(item, info)
  .zadd(user + _POSTLINE, time, item)
  .eval(SCRIPT_USER_PUSH_ITEM, 4, user, item, 0, time, print)
  .exec(callback);
};
...
exports.commentItem = function(user, item, info, callback) {
  shorten.nextId('comment', function(err, id) {
      ...
      .eval(SCRIPT_INCR_ITEM_RATE, 3, user, item, COMMENT_SCORE, print)
      .exec(callback);
...
exports.shareItem = function(user, item, info, callback) {
  shorten.nextId('share', function(err, id) {
      ...
      .eval(SCRIPT_INCR_ITEM_RATE, user, item, SHARE_SCORE, print)
      .exec(callback);
...
exports.likeItem = function(user, item, callback) {
  redis.sismember(user + _LIKE, item, function(err, islike) {
      if(islike === 0) {
        ...
        multi.eval(SCRIPT_INCR_ITEM_RATE, 3, user, item, incr, print).exec(callback);
        });
      } else {
...
exports.undoLikeItem = function(user, item, callback) {
  redis.sismember(user + _LIKE, item, function(err, islike) {
      if(islike === 1) {
        ...
        .eval(SCRIPT_INCR_ITEM_RATE, 3, user, item, - LIKE_SCORE, print)
        .exec(callback);
      } else {
        callback(new Error('data not found'));
      }
...
exports.undoDislikeItem = function(user, item, callback) {
  redis.sismember(user + _DISLIKE, item, function(err, isdislike) {
      if(isdislike === 1) {

        var multi = redis.multi()
        .srem(user + _DISLIKE, item)
        .eval(SCRIPT_INCR_ITEM_RATE, 3, user, item, - DISLIKE_SCORE)
        .exec(callback);
      } else {
        callback(new Error('data not found'));
      }
