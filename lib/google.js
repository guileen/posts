        ...
        function (err, data, response) {
          ...
          feeds = feeds.subscriptions.map(function(item) {
              if(item.id.indexOf('feed/') != 0) {
                console.log('========== feed ' + item.id + ' id is special =============');
              }
              console.log(item);
              var feedurl = item.id.substring(5);
              // format feed
              return {
                url: feedurl
              , key: 'feed:' + md5(feedurl)
              , title : item.title
          ...
          service.followFeeds(req.session.user, feeds, function(err, results) {
              if(err) return next(err);
